...
class WeatherRepositoryImpl(
    ...
    override suspend fun refreshWeather(location: Location?): CurrentWeather? {
        return when {
        ...
        return dbHelper.getAll().let {
            if (it.isEmpty()) {
                ...
                with(CurrentWeather(it.first(), getLastDownloadTime())) {
                    if (this.isInvalid()) {
                        throw WeatherRepository.WeatherDataConsistencyException("Weather data retrieved from the DB is empty or incomplete.-----\n${this.persistedWeather}\n-----")
                    } else {
                        this
                    }
                }
    ...
    private suspend fun doRefreshWeather(location: Location?): CurrentWeather {
        if (location == null) {
            throw WeatherRepository.LocationConsistencyException("Location is null and wasn't probably initialized yet.")
        }
        val weatherResult = try {
            weatherApi.getWeatherFromApi(location)
        } catch (e: Exception) {
            throw WeatherRepository.WeatherAPIException("Weather API has thrown an Exception: ${e.message}", e.cause)
        }
        ...
        if (!weatherResult.isInvalid()) {
            ...
            throw WeatherRepository.WeatherDataConsistencyException("Weather data obtained is empty or incomplete.-----\n$weatherResult\n-----")
        }
    }
